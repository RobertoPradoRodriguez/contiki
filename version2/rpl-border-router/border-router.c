...
PROCESS(border_router_process, "Border router process");  // PROCESS CONTROL BLOCK (PCB)
    ...
    {
        PROCESS_BEGIN();  // INICIA EL PROTOTHREAD DEL PROCESO
        // LLAMAS A LA FUNCION INIT DEL FICHERO httpd-simple.c
        // ARRANCAS EL SERVIDOR HTTP EN EL BORDER ROUTER
        httpd_init();

        while(1) {
            PROCESS_WAIT_EVENT_UNTIL(ev == tcpip_event); // ESPERA POR UN EVENTO BAJO ESA CONDICION
            // LLAMAS A LA FUNCION http_appcall DEL FICHERO httpd-simple.c
            httpd_appcall(data);
        }
        PROCESS_END();      // ACABA EL PROTOTHREAD DEL PROCESO
    }
    AUTOSTART_PROCESSES(&border_router_process,&webserver_nogui_process);
    ...
    /*  Esto son protosockets, solo funcionan con conexiones TCP
        Los protosockets utiizan protothreads. Cada uno vive solo
        dentro de un bloque de funcion.
    */
    static
...
#endif // FIN DEL "if" DEL WEBSERVER DE ARRIBA
...
void request_prefix(void)
{
    ...
    // uip_buf --> BUFFER donde guardas lo que vas a mandar  uip_buf[0] = '?';
    uip_buf[0] = '?';
    uip_buf[1] = 'P';
