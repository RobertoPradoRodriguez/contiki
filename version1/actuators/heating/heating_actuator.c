...
RESOURCE(leds, METHOD_POST | METHOD_PUT , "actuators/heating",
	"title=\"temperature: status=very-hot|hot|medium|down \";rt=\"Control\"");
...
void leds_handler(void* request, void* response, uint8_t *buffer, uint16_t preferred_size, int32_t *offset){
    ...
    if ((len=REST.get_post_variable(request, "status", &status))){
        ...
        if (strncmp(status, "very-hot", len)==0) {
            leds_on(LEDS_RED);
            leds_off(LEDS_GREEN);
            leds_off(LEDS_BLUE);
        }else if (strncmp(status,"hot", len)==0) {
            leds_off(LEDS_RED);
            leds_on(LEDS_GREEN);
            leds_off(LEDS_BLUE);
...
PROCESS(heating_actuator, "Erbium Example Server");
AUTOSTART_PROCESSES(&heating_actuator);

PROCESS_THREAD(heating_actuator, ev, data)
{
    PROCESS_BEGIN();
